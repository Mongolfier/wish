...
Copy `.env.example` to `.env` for local development. Production uses committed `apps/backend/.env.prod`; `SMTP_PASSWORD` and `DATABASE_URL` are injected on deploy from GitHub Actions secrets.
