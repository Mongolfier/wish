...
export interface RequestInterceptor {
	onFulfilled?: ((config: RequestConfig) => RequestConfig | Promise<RequestConfig>) | undefined;
	onRejected?: ((error: unknown) => unknown | Promise<unknown>) | undefined;
}

export interface ResponseInterceptor {
	onFulfilled?: ((response: Response, config: RequestConfig) => Response | Promise<Response>) | undefined;
	onRejected?: ((error: unknown, config: RequestConfig) => unknown | Promise<unknown>) | undefined;
}
...
export interface FetchService {
	...
	useResponse: (
		onFulfilled?: (response: Response, config: RequestConfig) => Response | Promise<Response>,
		onRejected?: (error: unknown, config: RequestConfig) => unknown | Promise<unknown>,
	) => void;
...
export const fetchService = (config: FetchClientConfig): FetchService => {
	...
	const fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
		...
		const requestConfig = await runRequestInterceptors({ input, init: requestInit });
	...
	const runResponseInterceptors = async (response: Response, config: RequestConfig): Promise<Response> => {
	...
	const invokeResponseInterceptor = async (
		response: Response,
		config: RequestConfig,
		onFulfilled?: (res: Response, config: RequestConfig) => Response | Promise<Response>,
		onRejected?: (error: unknown, config: RequestConfig) => unknown | Promise<unknown>,
	): Promise<Response> => {
		try {
			if (!onFulfilled) {
	...
	const handleFetchError = async (error: unknown, config: RequestConfig): Promise<Response> => {
